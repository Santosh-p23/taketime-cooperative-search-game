...
class GameState:
    ...
    #Lookahead check for zone6 exact-card-count requirement.
    def _zone6_feasible(self, target: int) -> bool:
        z6_after  = len(self.zones[6]) + (1 if target == 6 else 0)
        if z6_after > ZONE6_REQUIRED:
            return False
        needed = max(0, ZONE6_REQUIRED - z6_after)
        remaining = self.cards_remaining() - 1
        return needed <= remaining
    
    #Lookahead check for zone1 exact-one-white requirement.
    def _zone1_feasible(self, card: Card, target: int) -> bool:
        _, color = card
        z1_count = len(self.zones[1])
        z1_after = z1_count + (1 if target == 1 else 0)

        # Zone 1 must end with exactly one card.
        if z1_after > ZONE1_REQUIRED:
            return False

        # If we place in zone 1, it must be white.
        if target == 1 and color != ZONE1_COLOR:
            return False

        # If zone 1 is still empty after this move, ensure at least one white remains.
        if z1_after == 0:
            remaining_whites = sum(
                1 for hand in self.hands for _, c in hand if c == ZONE1_COLOR
            )
            if color == ZONE1_COLOR and target != 1:
                remaining_whites -= 1
    ...
    def is_legal_move(self, card: Card, zone: int) -> bool:
        ...
        if not self._zone1_feasible(card, zone):                return False
    ...
    def check_win(self) -> bool:
        ...
        if counts[0] != ZONE1_REQUIRED:                         return False
...
def heuristic(state: GameState) -> float:
    ...
    # 4. Zone 6 progress (reward being on track, penalize unrecoverable states)
...
def upper_bound(state: GameState) -> float:
    ...
    # (b2) Zone 1 must be exactly one white card
    if counts[0] > ZONE1_REQUIRED:
        return LOSS_SCORE
    if counts[0] == ZONE1_REQUIRED and any(c[1] != ZONE1_COLOR for c in state.zones[1]):
        return LOSS_SCORE
    if counts[0] == 0:
        remaining_whites = sum(
            1 for hand in state.hands for _, c in hand if c == ZONE1_COLOR
        )
        if remaining_whites == 0:
            return LOSS_SCORE
